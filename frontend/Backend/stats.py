...
def generate_stats(email):
    ...
    for row in results:
        stat_string = stat_string + f"""←++++++++++++++++++++++++++→ {row[0]} (Data was taken over a {row[1]} day period):\nTotal Carbon Footprint: {row[2]} 
        Tons.\nTheorertical Yearly Footprint: {row[3]} Tons.\nThis Theoretical Footprint is {row[4]} {row[5]}. Footprint for each
        category: Goods : {round(float(row[6]), 2)} Tons, Food : {round(float(row[7]), 2)} Tons, Water : {round(float(row[8]), 2)} Tons, Energy : {round(float(row[9]), 2)} Tons, Transport : {round(float(row[10]), 2)} tons.\n\n"""
